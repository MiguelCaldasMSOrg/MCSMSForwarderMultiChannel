...
class ProvisioningBundleTest {
    ...
    @Test
    fun rejectsUnknownPayloadValuesInsteadOfPartiallyImporting() {
        val payload = JSONObject()
            .put("masterEnabled", true)
            .put("allowedSender", "misspelled-field")
    ...
    @Test
    fun rejectsUnsupportedBundleVersion() {
        val envelope = JSONObject(encrypt(JSONObject().put("masterEnabled", true)))
            .put("version", 2)
        ...
        assertEquals("This configuration file version is not supported.", error.message)
    }

    @Test
    fun rejectsTamperedKdfMetadata() {
        val envelope = JSONObject(encrypt(JSONObject().put("masterEnabled", true)))
        envelope.getJSONObject("kdf").put("iterations", 600_001)
    ...
    fun rejectsEntriesThatCannotBeStoredLosslessly() {
        ...
        assertEquals(
            "Configuration value 'allowedSenders' contains an invalid entry at position 2.",
            error.message,
        )
    }

    @Test
    fun rejectsIncompleteChannelConfiguration() {
        val payload = JSONObject().put(
            "whatsApp",
            JSONObject()
                .put("enabled", true)
        ...
        assertEquals("Configuration value 'recipient' must be text.", error.message)
    }

    @Test
    fun rejectsMalformedEnvelopeEncoding() {
        val envelope = JSONObject(encrypt(JSONObject().put("masterEnabled", true)))
        envelope.getJSONObject("kdf").put("salt", "not-base64!")

        val error = assertThrows(ProvisioningException::class.java) {
            ProvisioningBundle.decrypt(envelope.toString(), passphrase)
        }

        assertEquals("Configuration value 'salt' is not valid Base64.", error.message)
    }
    ...
    fun rejectsExcessiveListEntries() {
        ...
        assertEquals(
            "Configuration value 'allowedSenders' cannot contain more than 1000 entries.",
            error.message,
        )
    }

    @Test
    fun preservesMultilineForwardingTemplate() {
        val payload = JSONObject().put(
            "filters",
            JSONObject().put("forwardTemplate", "From %s:\n%m"),
        )

        val result = ProvisioningBundle.decrypt(encrypt(payload), passphrase)

        assertEquals("From %s:\n%m", result.filters?.forwardTemplate)
    }

    @Test
    fun rejectsEmptyPayload() {
        val error = assertThrows(ProvisioningException::class.java) {
            ProvisioningBundle.decrypt(encrypt(JSONObject()), passphrase)
        }

        assertEquals("The configuration does not contain a supported setting.", error.message)
    }

    @Test
    fun rejectsOversizedBundleBeforeParsing() {
        val error = assertThrows(ProvisioningException::class.java) {
            ProvisioningBundle.decrypt(
                "x".repeat(ProvisioningBundle.MAX_BUNDLE_BYTES + 1),
                passphrase,
            )
        }

        assertEquals("The configuration file is too large.", error.message)
    }

    @Test
    fun rejectsShortPassphraseBeforeDecrypting() {
        val error = assertThrows(ProvisioningException::class.java) {
            ProvisioningBundle.decrypt("{}", "too-short".toCharArray())
        }
